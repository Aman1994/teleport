...
// defaultIsSuccess returns true if the provided error is non nil. This
// is the default value for Config.IsSuccessful if one is not provided.
...
func (c *Config) CheckAndSetDefaults() error {
	...
	c.Interval = jitter(c.Interval)
	c.TrippedPeriod = jitter(c.TrippedPeriod)
...
func jitter(duration time.Duration) time.Duration {
	...
	return (8 * duration / 9) + time.Duration(rng.Int63n(int64(duration))/9)
}
