...
func TestClient_RandomReconnect(t *testing.T) {
	t.Parallel()
	srv := startMockServer(t)
	ctx := context.Background()
	...
	state := clt.conn.GetState()
	errCh := make(chan error, 3)
	go func() {
		// close client to end reconnect loop and
		// close error channel when complete here
		// to end this test
		defer func() {
			errCh <- clt.Close()
			close(errCh)
		...
		// reconnect should first close the connection then create a new one
		clt.conn.WaitForStateChange(ctx, state)
		require.Equal(t, connectivity.Shutdown, clt.conn.GetState())

		select {
		case <-clt.reconnectCh:
		case <-time.After(30 * time.Second):
			errCh <- errors.New("client failed to reconnect")
			return
		}
		...
		// after reconnecting we should get back to the ready state
		errCh <- clt.waitForConnectionReady(ctx)
	}()

	randomlyReconnectLongLivedConnection(clt, time.Second, big.NewInt(-1))
